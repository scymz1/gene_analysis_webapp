...
def process_raw_anndata(row, h5_folder_path, npz_folder_path, scp, skip,
        ...
        # Fix path joining for checking file existence
        ...
        name = row.path.replace(".h5ad", "")
        proc_path = row.path.replace(".h5ad", "_proc.h5ad")
        if skip:
            proc_full_path = os.path.join(h5_folder_path, proc_path)
            if os.path.isfile(proc_full_path):
        ...
        adata_path = os.path.join(h5_folder_path, proc_path)
        adata.write(adata_path)

        arr = data_to_torch_X(adata.X).numpy()

        print(arr.max()) # this is a nice check to make sure it's counts
        filename = os.path.join(npz_folder_path, f"{name}_counts.npz")
        shape = arr.shape
        print(name, shape)
...
def get_species_to_pe(protein_embeddings_dir):
    """Get mapping between species and their protein embeddings."""
    # 获取当前文件的目录路径
    current_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    model_files_path = os.path.join(current_dir, "model_files")
    
    # 修改这行，使用完整路径
    extra_species = pd.read_csv(
        os.path.join(model_files_path, "new_species_protein_embeddings.csv")
    ).set_index("species").to_dict()["path"]
