...
class AnndataProcessor:
    def __init__(self, args, accelerator):
        ...
        # Set up the anndata
        self.adata_name = os.path.basename(self.args.adata_path)
        self.adata_root_path = os.path.dirname(self.args.adata_path)
        self.name = self.adata_name.replace(".h5ad", "")
        self.proc_h5_path = os.path.join(self.h5_folder_path, f"{self.name}_proc.h5ad")
        self.adata = None
        ...
        self.pe_idx_path = os.path.join(self.args.dir, f"{self.name}_pe_idx.torch")
        self.chroms_path = os.path.join(self.args.dir, f"{self.name}_chroms.pkl")
        self.starts_path = os.path.join(self.args.dir, f"{self.name}_starts.pkl")
        self.shapes_dict_path = os.path.join(self.args.dir, f"{self.name}_shapes_dict.pkl")
    ...
    def check_paths(self):
        ...
        if self.args.adata_path is None:
            ...
            self.args.adata_path = os.path.join(".", "data", "10k_pbmcs_proc.h5ad")
            figshare_download(
                "https://figshare.com/ndownloader/files/42706966",
                self.args.adata_path)
        print("self.args.model_loc", self.args.model_loc, self.args.model_loc is None)
        if self.args.model_loc is None:
            print("Using sample 4 layer model")
            self.args.model_loc = os.path.join(".", "model_files", "4layer_model.torch")
            # figshare_download(
...
def run_eval(adata, name, pe_idx_path, chroms_path, starts_path, shapes_dict,
    ...
    #### Run the model ####
    # Dataloaders
    dir_shape = args.dir_shape
    with open(dir_shape, 'rb') as f:
        shape_dict = pickle.load(f)

    directory_path = args.dir_data
    ...
    for dt in dt_list:
        ...
        dataset = MultiDatasetSentences(
                                        ...
                                        dt_path=os.path.join(directory_path, dt),
                                        shape=shape_dict[dt]
                                        )
        ...
        with torch.no_grad():
            for batch in pbar:
                ...
                _, embedding = model.forward(batch_sentences, mask=mask)  # 200*1280
        ...
        if accelerator.is_main_process:
            ...
            output_filename = f"{dt[:-4]}_embeds.npy"
            output_path = os.path.join(args.dir_ebds, output_filename)
            np.save(output_path, dataset_embeds)
