...
def dataloader(args):
    ...
    shape_dict_path = os.path.join(current_dir, 'dict_samples_shape.pkl')
    ...
    for dt in files_list_all:
        sd = shape_dict[dt]
        dt_path = os.path.join(args.data_path, dt)
        label_path = os.path.join(args.label_path, dt[:-11] + 'labels.npz')
