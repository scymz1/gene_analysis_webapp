...
class Gene2VecPositionalEmbedding(nn.Module):
    def __init__(self, dim, max_seq_len):
        super().__init__()
        gene2vec_weight = np.load(os.path.join(os.path.dirname(__file__), 'gene2vec_16906.npy'))
...
class PerformerLM(nn.Module):
    ...
    def forward(self, x, return_encodings = False, output_attentions = False, **kwargs):
        ...
        # a = self.token_emb(torch.tensor([0, 0, 0, 1, 3, 6, 1, 0]).cuda())  # del exp which >= 7
        # token and positional embedding
        x = self.token_emb(x)   #  exp
        if output_attentions:
            x.requires_grad_()    # used for attn_map output
        x += self.pos_emb(x)   # gene2vec
        x = self.dropout(x)
