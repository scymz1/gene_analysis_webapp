...
def main():
    ...
    for file_name in files_list:
        gexpr_feature = pd.read_csv(args.data_path + file_name, sep='\t', index_col=0,
                                    usecols=lambda column: column != 'Condition')
        print(gexpr_feature.columns)
        lbl = list(pd.read_csv(args.data_path + file_name, sep='\t', usecols=['Condition'])['Condition'])
        ...
        data_dict['x'] = x_all
        data_dict['x_padding'] = x_padding_all
        data_dict['pos_id'] = pos_id_all

        with open('/blue/qsong1/wang.qing/benchmark_dataset_API/scFoundation/samples/' + file_name[:-4] + '_samples.pkl', "wb") as p:
