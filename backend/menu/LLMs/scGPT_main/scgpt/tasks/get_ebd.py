...
from .data_collator_EBD import DataCollator
...
def embed_data(
    ...
    with open(os.path.join(output_dir, "samples", filename[:-4] + '_samples.pkl'), "wb") as p:
...
if __name__ == "__main__":
    ...
    output_dir='/blue/qsong1/wang.qing/benchmark_dataset_API/scGPT/samples'
    files_list = os.listdir(data_path)
    for file_name in files_list:
        embed_data(adata_or_file=os.path.join(data_path, file_name), model_dir=model_dir,
                use_fast_transformer=True,max_length=1200,
                batch_size=64, gene_col = "index", filename = file_name, output_dir=output_dir)
