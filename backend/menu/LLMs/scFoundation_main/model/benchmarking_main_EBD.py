...
parser.add_argument('--data_path', type=str, default='/media/volume/Minghao_webserver/gene_analysis_webapp/backend/menu/UCE_DB/labels&samples/20250213_042602/embeds/', help='')
parser.add_argument('--label_path', type=str, default='/media/volume/Minghao_webserver/gene_analysis_webapp/backend/menu/UCE_DB/labels&samples/20250213_042602/labels/', help='')
    ...
    for epoch in range(args.ep_num):
        ...
        with tqdm(train_data_loader, ncols=80, position=0, leave=True) as batches:
            for b in batches:  # sample
                ...
                pred = model(input_embeds.float().to(device))
