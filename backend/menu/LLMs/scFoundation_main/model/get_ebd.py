...
parser.add_argument('--data_path', type=str, default='/blue/qsong1/wang.qing/benchmark_dataset_API/lxndt_filter/', help='input data path')
...
def main():
    ...
    for file_name in files_list:
        csv_file_path=args.data_path+file_name
        #csv_file_path = csv_file_path[:-35] + 'Book2.csv'
        gexpr_feature=pd.read_csv(csv_file_path,sep='\t',index_col=0,usecols=lambda column: column != 'Condition')
        ...
        for i in tqdm(range(gexpr_feature.shape[0])):
            with torch.no_grad():
                ...
                if args.tgthighres[0] == 'f':
                    pretrain_gene_x = torch.tensor(tmpdata+[np.log10(totalcount*float(args.tgthighres[1:])),np.log10(totalcount)]).unsqueeze(0).cuda(0)
                elif args.tgthighres[0] == 'a':
                    pretrain_gene_x = torch.tensor(tmpdata+[np.log10(totalcount)+float(args.tgthighres[1:]),np.log10(totalcount)]).unsqueeze(0).cuda(0)
                elif args.tgthighres[0] == 't':
                    pretrain_gene_x = torch.tensor(tmpdata+[float(args.tgthighres[1:]),np.log10(totalcount)]).unsqueeze(0).cuda(0)
                else:
            ...
            if args.output_type=='cell':
                ...
                x = pretrainmodel.token_emb(torch.unsqueeze(x, 2).float(), output_weight = 0)  # exp+pos
        ...
        np.save('/blue/qsong1/wang.qing/benchmark_dataset_API/scFoundation/embeds/'+file_name[:-4]+'_embeds.npy',geneexpemb)
        # np.save('/blue/qsong1/wang.qing/benchmark_dataset_API/scFoundation/labels/'+file_name[:-4]+'_labels.npy', np.array(label))
